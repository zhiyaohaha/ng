...
var fixedDivTopHeight = $("#fixedDivTop").height(); //当前页面顶部的高度
    ...
    $.ajax({
        type: "GET",
        url: urlprefix + "/api/Template/GetDetailTemplateFormData",
        data: $.getAuth(),
        async: false,
        crossDomain: true,
        xhrFields: {
            ...
            if(res.code === "0") {
                ...
                $("#bindTarget").on("change", function(){
                    $.ajax({
                        ...
                        success: function(r){
                            if(r.code === "0") {
                                domElementUi(activeId, $("#displayType").val(), r.data);
                            }
                        }
                    })
                })
                ...
                if(detailId){  //从url判断是 新增模板还是修改模板
                    ...
                    $.ajax({
                        type: "GET",
                        url: urlprefix + "/api/Template/GetDetailTemplateFormDetail",
...
function domElementUi (id , type , data) {
    var html = "";
    if(type === "HtmlDomDisplayType.Select"){
...
$(document).on("click", ".element-wrap, .dom-panel", function(){
    ...
    // $("#bindPipe").val(objData[id].bindPipe);
...
function bindCmds(data) {
    ...
    data.forEach(function(el,index){
        ...
        if(el.name || el.formTemplate ||   el.triggerUrl || el.bindParamFields || el.triggerWhere){
            if(el.triggerWhere){
...
function objdataChildrens(that,objDataId){
    ...
    that.children().each(function(index, element){
        if($(this).hasClass("dom-panel")){   //循环单个组件--二级面板
            // console.log('面板')
            objdataChildrens($(this).children(".dom-panel-content"),objDataId);
            // objDataId.childrens =  objDataId.childrens.concat(childPanel);
...
$("#formDomTarget").on("click", ".copy-panel-field",function(){
    ...
    var id = $(this).parent().attr("id");
    objdataChildrens($(this).parent(),objData[id]);  //循环单个组件--按钮域和二级面板，其它组件不需要
    objData[id].ui.columns = parent.data("col");        //复制行数
    if(!parent.parent().hasClass("dom-panel-content")){   
        dragCreateDom_Panel_ButtonRegion(objData[id].ui.displayType,parent.parent(".element-wrap"),objData[id]);  //按钮域下面的按钮复制
    }else{
        dragCreateDom_PanelBody(objData[id].ui.displayType,parent.parent().parent(".dom-panel"),objData[id]); 
    }
})
...
$("#bindField, #bindTitle").droppable({
    drop: function( event, ui ) {
        ...
        $("#bindTitle").val(bindTitle);
         // $("#"+activeId).find("span").html(bindTitle);  面板组件拖入值时，子组件label都会被赋予 面板的拖入值
        // $("#"+activeId+">span").html(bindTitle);  
...
function displayDOM(key, id) {
    ...
    switch(key) {
        case "HtmlDomDisplayType.ButtonRegion": 
            ...
            break;
        case "HtmlDomDisplayType.StaticText":
            return $(returnDom("静态文本",`<span></span>`, id));
        case "HtmlDomDisplayType.StaticTextArea":
            return $(returnDom("静态文本域",`<span></span>`, id));
            break;
        case "HtmlDomDisplayType.Panel":
            return $(`<div class="dom-panel" id="${id}"  data-col="2" style="width:100%;min-height:100px;float:right;">
                    <button class="btn btn-default btn-xs copy-panel" title="复制该组件"><i class="fa fa-files-o"></i></button>
        ...
        case "HtmlDomDisplayType.Table":
            return $(returnDom("表格",``,id));
            break;
        case "HtmlDomDisplayType.Image":
            return $(returnDom("图片",`<span></span>`, id));
            break;
        default :
            return "";
            break;
...
function returnDom(type,html, id) {
    var displayHtml = ``;
    if(type=="表格"){
        displayHtml =`
...
function saveCmds(){
    var arr = [];
    //每次新增一个命令栏组结果，都会新增一个空的命令栏组。所以计算时应该排除没有值的命令栏组(空的)

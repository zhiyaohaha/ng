...
function domElementUi (id , type , data) {
    ...
    if(type === "HtmlDomDisplayType.Select" || type === "HtmlTemplate.Select"){
        ...
        for(var i = 0; i < data.length; i++){
            html += `<label><input type="radio" value="${data[i].value}">${data[i].label}</label>`;
        }
        $("#"+id).find("label, input").remove();
        $("#"+id).append(html);
    }
...
$(document).on("click", ".element-wrap, .dom-panel", function(){
    ...
    if($("#bindMethod").val() && $("#bindTarget").children().length == 0){  //编辑状态时有值，但是无optiobns选项
        $.ajax({
            type: "GET",
            url: urlprefix + "/api/Template/GetBindTarget",
...
function saveTemplate() {
    ...
    var formTemplateOperateUrl = '';        
    if(detailId){    //编辑模板
        formTemplateOperateUrl  = 'Update';
        data.id = detailId    
    }else{         //新增模板
        formTemplateOperateUrl  = 'Add';  
    }
    console.log(formTemplateOperateUrl)
    console.log(data)
    // $.ajax({
    //     type: "POST",
    //     url: urlprefix + "/api/Template/"+formTemplateOperateUrl+"FormTemplate",
    //     data: $.postAuth(data),
    //     crossDomain: true,
    //     xhrFields: {
    //         withCredentials: true

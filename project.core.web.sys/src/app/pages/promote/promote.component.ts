...
@Component({
  ...
  animations: [fadeIn],
  providers: [TdLoadingService, PromoteService]
})
export class PromoteComponent implements OnInit {
  ...
  getParamsList(params) {
    this.promoteService.getParamsList(params)
      .subscribe(res => {
        if (res.code === "0") {
          let r = res;
   ...
   */
  rowClickEvent($event) {
    this.promoteService.getEditParams({ id: $event.row.id, level: "second" })
      .subscribe(r => {
  ...
  showLevel(e, i) {
    this.level2[i].checked = !this.level2[i].checked;
  }
}
